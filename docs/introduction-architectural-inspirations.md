...
Hyrest does not solve all problems your project might have and does not come with an opinionated set of peer-dependencies.
Hyrest is framework-agnostic and neither enforces what to use in the frontend, for networking, a database or a project structure to use.
...
The following rough layers have proven useful not only for JavaScript frameworks, [but for other frameworks, too](http://exploreflask.com/en/latest/organizing.html):
...
- **State-Management**: Make sure to use a [state-management](https://stateofjs.com/2017/state-management/other/) library in the frontend.
- **Rendering**: Decide for one of the various modern frontend frameworks ([I recommend React](https://reactjs.org/)). Pay attention to keep this thin and clean of logic. Think about using a [component library](https://hackernoon.com/23-best-react-ui-component-libraries-and-frameworks-250a81b2ac42) to further separate design from UX and layouting.
...
[ORM](https://en.wikipedia.org/wiki/Object-relational_mapping)s are very popular outside of the JavaScript ecosystem ([SQLAlchemy](https://www.sqlalchemy.org/), [Hibernate](http://hibernate.org/orm/), [ODB](https://codesynthesis.com/products/odb/), [Diesel](http://diesel.rs/), [Doctrine](https://www.doctrine-project.org/)).

[ORM](https://en.wikipedia.org/wiki/Object-relational_mapping)s provide a nice abstraction over different databases.
This can serve as an additional layer between your database and models and your controllers.
When using an ORM with models explicitly declared as classes ([Typeorm](http://typeorm.io)), your models can be used for the database and as interfaces for the controllers, which can come in pretty handy.

## Directory structure

Many frameworks come with scaffolding tools or boilerplates creating a given directory structure.
Hyrest does not provide something like that.
Instead, much like the [Flask's guide about Larger applications](http://flask.pocoo.org/docs/0.12/patterns/packages/), an inspiration for structuring your project will be provided here.
...
Code in `server/` and `web/` can import from `common/` but not the other way around.
Neither can `server/` and `client/` import from each other.

### Multiple frontends

When implementing a project with multiple frontends (e.g. web, Android and iOS), split out the `store/` and `utils/` directory into a `ui/` directory and keep a separate directory for each frontend. E.g.:
...
You will probably iterate on your whole project heavily in the initial phases, but after a while your application tier and persistence tier will settle down and act as a foundation for your project.
...
## Don't mix up Markup and data flow
...
> Rearranging components will likely be a common task after your initial application has been written, as [UX is a continuous process](https://ubilabs.net/en/news/ux-design-a-continuous-process-2016-02-02) affecting the structure of your markup.

The way the data reaches your components should be orthogonal to the structure of your markup.
Only use properties for the absolute minimum set of information available (for a card representing a user it might just be the id of the user), and grab anything else from the state directly from within the component.
...
It depends on a *Provider* being the parent of all components consuming the data which is (in my personal opinion) still a flawed concept.
...
Take a look at dependency injectors as a structure for your project to avoid singletons, get control over startup and teardown of your backend and solve the problem of accessing your state-management from your components.
