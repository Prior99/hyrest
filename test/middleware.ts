...
test("The `hyrest` middleware handles invalid requests correctly", async () => {
    ...
    class TestController4 {
        @route("GET", "/user/:id")
        public getTest(
                @is(DataType.int) @param("id") id: number,
                @is(DataType.float).validate(required) @query("search") search: number) {
        ...
        @route("POST", "/echo")
        public postEcho(@is().schema({ email: is(DataType.str).validate(email) }) @body() user: any) {
...
test("The `hyrest` middleware performs a context validation", async () => {
    ...
    class TestController6 {
        @route("GET", "/get/:id")
        public getGet(@param("id") @is(DataType.str).validateCtx(ctx => ctx.validate1) id: string) {
            return ok();
        }

        @route("GET", "/get/array/:id")
        public getArray(@param("id") @is(DataType.str).validateCtx(ctx => [ctx.validate2, ctx.validate3]) id: string) {
            return ok();
        }
    }
